...
func (dc *DefaultCommand) Start(data *commander.StartData) error {
	wsServer, err := go_config.ConfigManagerInstance.GetString("ws-server")
	if err != nil {
		return err
	}
	contractAddress, err := go_config.ConfigManagerInstance.GetString("contract-address")
	if err != nil {
		return err
	}
	telegramToken, err := go_config.ConfigManagerInstance.GetString("telegram-token")
	if err != nil {
		return err
	}
	if telegramToken == "" {
	...
	chatIdStr, err := go_config.ConfigManagerInstance.GetString("chat-id")
	if err != nil {
		return err
	}
	if chatIdStr == "" {
	...
	listenMethods := make([]string, 0)
	go_config.ConfigManagerInstance.MustGet("listen-methods", &listenMethods)

	notifyStr, err := go_config.ConfigManagerInstance.GetString("notify")
	if err != nil {
		return err
	}
	...
	go func() {
		...
		for range timer.C {
			err := telegramRobot.TelegramSender().SendMsg(telegram_sender.MsgStruct{
				ChatId: chatId,
				Msg:    fmt.Sprintf("监控中，一切正常（合约地址：%s）", contractAddress),
			}, 0)
			if err != nil {
	...
	for {
		...
		case result := <- resultChan:
			...
			isDead := false
			for _, listenMethod := range listenMethods {
				if strings.Contains(methodStr, listenMethod) {
					isDead = true
				}
			}
			if isDead {
				methodStr = fmt.Sprintf(`
⚠️紧急通知： %s 被调用，迁移合约变动，资产风险非常高，为确保资产安全，请立刻联系陈旭（手机号：13575724011）提出 pancake 所有资产，并联系继勇（手机号：18317042249）查看情况
 ` + notifyStr, methodStr)
			} else {
				methodStr = fmt.Sprintf(`
通知： %s 被调用，但无风险，无需任何动作，工作日提醒继勇确认即可
`, methodStr)
			}
			err := telegramRobot.TelegramSender().SendMsg(telegram_sender.MsgStruct{
				ChatId: chatId,
